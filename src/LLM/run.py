...
def get_keywords(model, article) -> list:
    from src.utils import read_content
...
def get_score(model, article, system_prompt="") -> int:
    prompt = f"\n\n{article}"    
    ...
    try:
        ...
        raise ValueError("Failed to get score after multiple attempts.")
...
def main():
    ...
    score = 0
    score = max(score, get_score(model, article, SCORE_HEADLINE_PROMPT))
    score = max(score, get_score(model, article, SCORE_TECH_PROMPT))
    score = max(score, get_score(model, article, SCORE_TALENT_PROMPT))
    score = max(score, get_score(model, article, SCORE_INTERNATIONAL_PROMPT))
    score = max(score, get_score(model, article, SCORE_EDUCATION_PROMPT))
    print(score)
