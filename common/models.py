...
url_type = Annotated[str, mapped_column(String(2048), nullable=False)]
...
cata_fk = Annotated[
    int, mapped_column(ForeignKey(Catagory.id), index=True, nullable=False)
]
...
class Site(Base):
    ...
    name: Mapped[str]
    url: Mapped[url_type]
    cata_id: Mapped[cata_fk]
    cata = relationship(Catagory, back_populates="sites")
